...
const AddHafalan = () => {
  ...
  const { students, addHafalanBaruLog, addPersiapanUjianLog, addUjianLog, getLastHafalanLog, getLastPersiapanLog, getLastUjianLog, fetchStudentData } = useAppStore();
  ...
  // Apply last log values when type changes
  useEffect(() => {
    if (!form.student_id) return;

    if (form.type === 'setoran') {
      const lastLog = getLastHafalanLog(form.student_id);
      if (lastLog) {
        setForm(prev => ({
          ...prev,
          juz_id: lastLog.juz_id,
          from_page: lastLog.from_page,
          from_line: lastLog.from_line,
          to_page: lastLog.to_page,
          to_line: lastLog.to_line,
          note: '',
        }));
      }
    } else if (form.type === 'persiapan_ujian') {
      const lastLog = getLastPersiapanLog(form.student_id);
      if (lastLog) {
        setForm(prev => ({
          ...prev,
          juz_id: lastLog.juz_id,
          exam_type: lastLog.exam_type,
          juz_part: lastLog.juz_part,
          note: '',
        }));
      }
    } else if (form.type === 'ujian') {
      const lastLog = getLastUjianLog(form.student_id);
      if (lastLog) {
        setForm(prev => ({
          ...prev,
          juz_id: lastLog.juz_id,
  ...
  }, [form.type, form.student_id, getLastHafalanLog, getLastPersiapanLog, getLastUjianLog]);
